...
# The subset of GitHub's REST API and OAuth token exchange this product calls: authorizing the Administrator's account, listing the repositories they can grant access to, and polling a repository's open issues labeled "critical".
public isolated client class Client {
    ...
    # + return - The access token for the authorizing user 
    resource isolated function post login/oauth/access_token(oauth_access_token_body payload, map<string|string[]> headers = {}) returns inline_response_200|error {
        string resourcePath = string `/login/oauth/access_token`;
    ...
    # Get the GitHub user who authorized the app
    #
    # + headers - Headers to be sent with the request 
    # + return - The authenticated GitHub user 
    resource isolated function get user(map<string|string[]> headers = {}) returns User|error {
        string resourcePath = string `/user`;
        return self.clientEp->get(resourcePath, headers);
    }

    # List repositories the authorizing user can grant access to
    #
    # + headers - Headers to be sent with the request 
    # + return - Repositories visible to the authenticated user 
    resource isolated function get user/repos(map<string|string[]> headers = {}) returns Repository[]|error {
        string resourcePath = string `/user/repos`;
        return self.clientEp->get(resourcePath, headers);
    }

    # List a repository's issues, filterable by label and state
    #
    # + headers - Headers to be sent with the request 
    # + queries - Queries to be sent with the request 
    # + return - Issues matching the filter 
    resource isolated function get repos/[string owner]/[string repo]/issues(map<string|string[]> headers = {}, *ListRepositoryIssuesQueries queries) returns Issue[]|error {
...
public type Issue_labels record {
    string name?;
};

public type Repository_owner record {
    string login?;
};

# Represents the Queries record for the operation: listRepositoryIssues
public type ListRepositoryIssuesQueries record {
    "open"|"closed"|"all" state = "open";
    # Comma-separated label names, e.g. "critical"
    string labels?;
};
...
public type oauth_access_token_body record {
    string client_id;
